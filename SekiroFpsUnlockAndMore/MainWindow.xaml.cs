    ...
    public partial class MainWindow : Window
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (_settingsService.ApplicationSettings.cameraAdjustNotify)
                this.sbInput.Text = _settingsService.ApplicationSettings.peasantInput ? "控制器输入" : "鼠标输入";
            ...
            _dispatcherTimerGameCheck.Tick += new EventHandler(async (object s, EventArgs a) =>
            {
                ...
                if (result)
                {
                    UpdateStatus("正在扫描游戏进程……", Brushes.Orange);
        ...
        private void ComponentDispatcherThreadFilterMessage(ref MSG msg, ref bool handled)
        {
            ...
            switch (id)
            {
                case HOTKEY_ID_PATCH:
                    PatchGame();
                    break;
                case HOTKEY_ID_FOV_DEC:
                    cbFov.IsChecked = true;
                    BFovLower_HotKey();
                    break;
                case HOTKEY_ID_FOV_INC:
                    cbFov.IsChecked = true;
                    BFovHigher_HotKey();
                    break;
                default:
        ...
        private Task<bool> CheckGame()
        {
            ...
            if (gameIndex < 0)
            {
                UpdateStatus("无法访问游戏进程……", Brushes.Red);
                LogToFile("无法访问游戏进程……");
            ...
            UpdateStatus("游戏正在初始化数据……", Brushes.Orange);
            return Task.FromResult(false);
        }
        ...
        private bool PatchFramelock(bool showStatus = true)
        {
            ...
            if (showStatus) UpdateStatus(DateTime.Now.ToString("HH:mm:ss") + " 已修改！", Brushes.Green);
            return true;
        }
        ...
        private void PatchGame()
        {
            ...
            if (results.IndexOf(true) > -1)
                UpdateStatus(DateTime.Now.ToString("HH:mm:ss") + " 已修改！", Brushes.Green);
            else
                UpdateStatus(DateTime.Now.ToString("HH:mm:ss") + " 已还原！", Brushes.White);
        ...
        private void BFovLower_HotKey()
        {
            Int32.TryParse(this.tbFov.Text, out int fov);
            if (fov < -90)
                this.tbFov.Text = "-95";
            else if (fov > 95)
                this.tbFov.Text = "95";
            else
            {
                ...
                else if ((mod = fov % 10) < 5)
                    this.tbFov.Text = (fov - mod).ToString();
                else
                    this.tbFov.Text = (fov - mod + 5).ToString();
            }
            if (this.cbFov.IsChecked == true) PatchFov();
        }
        ...
        private void BFovHigher_HotKey()
        {
            Int32.TryParse(this.tbFov.Text, out int fov);
            if (fov > 90)
                this.tbFov.Text = "95";
            ...
            else
            {
                ...
                if (0 == mod)
                    ...
                    this.tbFov.Text = (fov - mod + 10).ToString();
            }
            if (this.cbFov.IsChecked == true) PatchFov();
        }

        private void BFov100_Click(object sender, RoutedEventArgs e)
        {
            this.tbFov.Text = "95";
            if (this.cbFov.IsChecked == true) PatchFov();
        }
        ...
        {
            Int32.TryParse(this.tbGameSpeed.Text, out int gameSpeed);
            if (gameSpeed < 5)
                this.tbGameSpeed.Text = "0";
            else if (gameSpeed > 200)
                this.tbGameSpeed.Text = "200";
            else
            {
                ...
                else if ((mod = gameSpeed % 10) < 5)
                    this.tbGameSpeed.Text = (gameSpeed - mod).ToString();
                else
                    this.tbGameSpeed.Text = (gameSpeed - mod + 5).ToString();
            }
            if (cbGameSpeed.IsChecked == true) PatchGameSpeed();
        }
        ...
        private void BGsHigher_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                if (0 == mod)
                    ...
                    this.tbGameSpeed.Text = (gameSpeed - mod + 10).ToString();
            }
            if (cbGameSpeed.IsChecked == true) PatchGameSpeed();
        }
        ...
        {
            Int32.TryParse(this.tbPlayerSpeed.Text, out int playerSpeed);
            if (playerSpeed < 5)
                this.tbPlayerSpeed.Text = "0";
            else if (playerSpeed > 200)
                this.tbPlayerSpeed.Text = "200";
            ...
            {
                int mod = playerSpeed % 5;
                if (0 == mod)
                    this.tbPlayerSpeed.Text = (playerSpeed - 5).ToString();
                else if ((mod = playerSpeed % 10) < 5)
                    this.tbPlayerSpeed.Text = (playerSpeed - mod).ToString();
                else
                    this.tbPlayerSpeed.Text = (playerSpeed - mod + 5).ToString();
            }
            if (cbPlayerSpeed.IsChecked == true) PatchPlayerSpeed();
        }
        ...
        private void BPsHigher_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                if (0 == mod)
                    ...
                    this.tbPlayerSpeed.Text = (playerSpeed - mod + 10).ToString();
            }
            if (cbPlayerSpeed.IsChecked == true) PatchPlayerSpeed();
        }
        ...
        private const int HOTKEY_ID_PATCH = 0x9001;
        private const int HOTKEY_ID_FOV_INC = 0x9002;
        private const int HOTKEY_ID_FOV_DEC = 0x9003;
